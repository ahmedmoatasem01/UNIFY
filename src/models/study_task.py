"""
Study Task Model
Represents a task within a study plan with hierarchical structure support
"""
...
class StudyTask:
    def __init__(self, Task_ID: Optional[int] = None, Plan_ID: int = 0,
                 ...
                 Suggested_Resources: Optional[str] = None,
    ...
    def __repr__(self):
        return f"<StudyTask(Task_ID={self.Task_ID}, Task_Title='{self.Task_Title}', Status='{self.Status}')>"
    
    def get_resources_list(self):
        """Parse and return suggested resources as a list"""
        if not self.Suggested_Resources:
            return []
        try:
            return json.loads(self.Suggested_Resources)
        except:
            return []
    
    def set_resources_list(self, resources_list):
        """Set suggested resources from a list"""
        self.Suggested_Resources = json.dumps(resources_list)
    ...
    def to_dict(self):
        ...
        return {
            ...
            'Due_Date': self.Due_Date.isoformat() if isinstance(self.Due_Date, datetime) else str(self.Due_Date) if self.Due_Date else None,
            'Priority': self.Priority,
            'Status': self.Status,
            'Suggested_Resources': self.get_resources_list(),
